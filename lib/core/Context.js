...
Context.registerComponent = function (group, name, ...clazz)
{
    for (let cls of clazz)
    {
        ...
        assert (cls.prototype instanceof adapter, `The '${group}' must be an instance of ${adapter.name}.`);
...
function init ()
{
    for (let d in COMPONENTS)
    {
        for (let f of COMPONENTS[d])
        {
            ...
            Context.registerComponent (d, f, clazz);
        }
    }
}
